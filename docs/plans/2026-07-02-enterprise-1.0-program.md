...
| —   | Adversarial e2e (Alice/Bob) — **DONE 2026-07-03** | Two mapped users; assert A cannot read B's files via terminal/explorer/editor/git/search/upload/task, members can't touch onboarding/admin, unmapped/tunnel actors denied. **Landed `150df6d`…`94c53e4` (plan `2026-07-03-alice-bob-isolation-e2e.md`): dedicated isolation harness (mock CF edge, offline owner seed, real broker) + 55-assert spec, green live (dtalice/dtbob) + `isolation-e2e` CI job. Fixed 4 product bugs the e2e surfaced (S1 member terminal grant rootId-keying, brokered terminal start-dir, search isolation deny = latent service-account-search hole, terminal-list canonical ownerId) + 3 Codex integrated-pass mediums.** Runs in CI from now on.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `tests/`                                                                                                                                              | Sonnet writes, Opus verifies the assertions are real                                                                                   |
...
**TRACK D COMPLETE on `dev` 2026-07-06** — all six slices delivered per
`docs/plans/2026-07-05-track-d-ide-ux.md` (Codex-validated rev 3 + per-slice D4 Codex
review + integrated pre-finalization pass); delivery record in that doc's §10.
Deviations of note: D4 shipped **without ripgrep** (broker `search` profile wired,
grep engine kept — deploy-pipeline rationale in the plan §6) and replace is
**literal-only v1**; D5 defers format-on-save to 1.1. Awaiting owner testing on dev
before the promotion PR to `main`.
...
| #               | Slice                         | What                                                                                                                                                                                                                                                                                                  | Tier                                                                           |
| --------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| D1 ✅ `551c9e7` | Quick-open (Ctrl+P)           | Fuzzy file finder in the palette (today only "Go to Directory", `app.js:7896-7906`); enumerates only actor-approved realpath roots. Add editor/git/Toggle-IDE commands to the palette.                                                                                                                | Sonnet                                                                         |
| D2 ✅ `9c372d3` | Renderer + fonts              | Adopt `@xterm/addon-webgl` (+ `addon-unicode11`) with canvas fallback and no terminal-semantics change. **Follows A3's regression tests — it is not the A3 fix.**                                                                                                                                     | Sonnet, Opus review                                                            |
| D3 ✅ `06f9c9d` | Merge-conflict handling       | Conflicted files get their own SCM group + accept-current/incoming/both actions running as mapped uid with approved cwd (today conflicts fall into "Changes" with a CSS class, `git-scm.js:31,38-49`). 3-way editor: 1.1.                                                                             | Sonnet                                                                         |
| D4 ✅ `fbf2796` | Workspace search v2 + replace | ~~`@vscode/ripgrep`~~ broker `search` profile (grep engine kept; plan §6 deviation); **replace-in-files is a high-risk write surface, not a UX slice**: mandatory preview, size/binary limits, symlink-race protection, atomic writes preserving owner/mode, audited, as mapped uid. Literal-only v1. | **Security mini-design + Codex review before code**; Sonnet codes, Opus review |
| D5 ✅ `86029f0` | Editor breadth                | More CodeMirror language modes (today 7), autosave, ~~format-on-save~~ (deferred to 1.1 — deliberate, plan §3), `@codemirror/search` in-editor.                                                                                                                                                       | Sonnet, low effort                                                             |
| D6 ✅ `a6d001a` | Settings/UI polish            | Styled checkboxes (QA P2), consistent focus states, empty/loading states pass, contrast check.                                                                                                                                                                                                        | Sonnet, low effort                                                             |
...
## 10. Rev 3 change log (B1 design validation, 2026-07-02)

B1+E1 written and Codex-validated same day (14 findings, all incorporated —
`2026-07-02-b1-identity-isolation-storage-design.md` §6). Program-level corrections: no
DeckTerm user may ever map to the service account (bootstrap-admin→`deploy` is dead — B1
§2.1); fs containment enforced fd-based in the run-as-user helper, not by app-side realpath
(B4 must implement `openat2`/`RESOLVE_BENEATH`); enabling any multiuser flag gates on an
audited review of legacy wildcard grants (B3); `cloudflare-tunnel` fails closed on
non-loopback binds (B2); recordings capture server-side, separate from the user-writable
reconnect spool (C4); multiuser mode requires an external audit-anchor sink (B6/C2). B5
landed on `dev` (`c48d85e`) with its own foundation test invocation, live-verified on 4174.
