...
| A6  | tsc gate + CI e2e harness        | Fix the 8 pre-existing `tsc --noEmit` errors (`server.ts:856,885,1531`, `task-runner.ts:575,580`, `foundation-actors.test.ts:12`, `onboarding-doctor.ts:209,226`), add `tsc` as a hard CI gate. Diagnose + fix the CI-only smoke-e2e failure (~2 WS connections in the headless runner), remove `continue-on-error` from `ci.yml` + `deploy-main.yml`. **M0 does not exit without this** — if the e2e root cause resists, escalate model/effort, don't waive the gate. | `backend/*`, `.github/workflows/*`, `tests/`                                                                            | M–L  |
