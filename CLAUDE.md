...
- `server.ts` — **~4.3k lines, the entire HTTP/WS surface.** Seams: `createWebApp()` (Hono routes), `startWebServer()` (`Bun.serve` + WS handlers), `reconcileSessionsOnStartup()`. All terminal/file/git/task/onboarding routes live here.
